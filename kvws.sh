...
investovat () {
    ...
    if [[ ! $investice = 6 ]] && [[ ! $investice = 10 ]]; then
        ...
        if ((penize_za_kolo > 4)) && [[ ! $obtiznost = "E" ]] || ((penize_za_kolo > 9)); then
            echo -e "Už jsi investoval až moc peněz!\n"
            err=1
        fi
    elif [[ $investice = 10 ]]; then
        if ((penize_za_kolo > 3)) && [[ ! $obtiznost = "E" ]] || ((penize_za_kolo > 8)); then
            echo -e "Už jsi investoval až moc peněz!\n"
            err=1
        fi
    fi
    if [[ $err = 0 ]] && ((penize < investice)); then
        echo -e "NEMÁŠ DOSTATEK FINANCÍ!\n"
    elif [[ $err = 0 ]]; then
        penize=$((penize - investice))
        penize_za_kolo=$((penize_za_kolo + (investice / 5)))
        if ((penize_za_kolo < 5)); then
...
dalsi_kolo () {
    ...
    for kolo in {0..50..5}; do
        [[ ! $kolo = "$kola" ]] && continue
        if [[ $obtiznost = "E" ]] && [[ ! $((kolo % 10)) ]]; then
            :
        elif ((vojaci < 2000)) && [[ $obtiznost = "H" ]] || ((vojaci < 1000)); then
